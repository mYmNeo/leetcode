...
private:
    void bfs_replace(std::vector<std::vector<char> > &board, std::pair<int, int> cur, std::pair<int, int> &end)
    {
        ...
        while (!next.empty()) {
            ...
            if (cur.first < 0 || cur.first == end.first || cur.second < 0 || cur.second == end.second) {
                continue;
            }
