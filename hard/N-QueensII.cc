...
private:
    void place_a_queen(int &num, std::vector<std::vector<bool> > &chess, int put, int row)
    {
        if (put == row) {
            ++num;
            return;
    ...
    bool check_valid(std::vector<std::vector<bool> > &chess, const int &row, const int &col, const int &n)
    {
        for (int i = 0; i < n; ++i) {
            if (chess[i][col]) {
                return false;
